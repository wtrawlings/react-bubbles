...
Token is added as a header to each request for information from a secure server. The Token is issued to the user when they successfully login to the system.

- [x] What steps can you take in your web apps to keep your data secure?

Protected Routes that can only be accessed with a valid token and the authentication headers with our api calls.

- [x] Describe how web servers work.

The server stores information. Clients make requests to the server so the server can process and hand out (serve) information in various packets. These packets are the web pages we make.  
